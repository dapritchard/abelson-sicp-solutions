...
;; the main idea here is that we make use of the identity
;;
;;     a * b = (2 * a) * (b / 2)
;;
;; when `b` is even, and the identity
;;
;;     a * b = a * (b - 1 + 1) = a + (a * (b - 1))
;;
;; when `b` is odd.  In either case we reduce the value of `b` at each call
;; until `b` has the value of 0, in which case the procedure returns a value of
;; 0.

(define (prod a b)
  (cond ((= b 0) 0)
	((even? b) (prod (double a) (half b)))
	(else (+ a (prod a (- b 1))))))
