...
Create the src_lisp[:exports code]{smooth} function.
#+ATTR_LATEX: :options morekeywords={define,lambda}
#+BEGIN_SRC scheme
...
\begin{figure}[htb]
  ...
  \vspace{3mm}
  \includegraphics[width=1\textwidth]{figures/sin-f.pdf}
\end{figure}
#+END_EXPORT
...
<a name="example-function-plot">
  <p class="aligncenter">
    ...
    <img src="figures/sin-f.svg" align="middle" />
  </p>
</a>
#+END_EXPORT
...
periodicities of $2\pi/11 \approx 0.57$ and $2pi/7 \approx 0.89$.  With these
values in mind, we set $dx$ to 0.05 which meant that it took the average over a
window of width 0.7 since we had 15 total points.
...
\begin{figure}[htb]
  ...
  \vspace{3mm}
  \includegraphics[width=1\textwidth]{figures/smoothed-sin.pdf}
\end{figure}
#+END_EXPORT
...
<a name="smoothed-function-plot">
  <p class="aligncenter">
    ...
    <img src="figures/smoothed-sin.svg" align="middle" />
  </p>
</a>
#+END_EXPORT
